...
processMod options ovs extraPaths name = do
  ...
  (gir, girDeps, typelibMap) <- loadGIRInfo (optVerbose options) nameWithoutVersion version extraPaths
                    (girFixups ovs)
...
dump options ovs name = do
  let (nameWithoutVersion, version) = solveNameVersion ovs name
  (doc, _, _) <- loadGIRInfo (optVerbose options) nameWithoutVersion version (optSearchPaths options) []
