...
unsafeManagedPtrGetPtr = unsafeForeignPtrToPtr . newtypeToForeignPtr

-- | Same as 'unsafeManagedPtrGetPtr', but includes a cast to a
-- different target pointer type.
unsafeManagedPtrCastPtr :: forall a b. ForeignPtrNewtype a => a -> Ptr b
unsafeManagedPtrCastPtr x = let p = newtypeToForeignPtr x :: ForeignPtr a
...
touchManagedPtr x = let p = newtypeToForeignPtr x :: ForeignPtr a
