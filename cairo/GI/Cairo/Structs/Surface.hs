...
instance (info ~ ResolveSurfaceMethod t Surface, O.MethodInfo info Surface p) => O.IsLabelProxy t (Surface -> p) where
    fromLabelProxy _ = O.overloadedMethod (O.MethodProxy :: O.MethodProxy info)

#if MIN_VERSION_base(4,9,0)
instance (info ~ ResolveSurfaceMethod t Surface, O.MethodInfo info Surface p) => O.IsLabel t (Surface -> p) where
